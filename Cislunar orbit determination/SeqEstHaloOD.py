...
class seq_est:
    def __init__(self, X0, tspan):
        ...
        self.R_obs = Earth_observer(0, 45, (2.361e6 / (2 * np.pi)) * self.t)
        # print(rm)
        # plt.plot(rm[:,0], rm[:,1])
        # plt.show()
        #
    ...
    def run(self):
        ...
        pos_std = 100 #m
        vel_std = 10 #mm/sec
        P0_pos = ((pos_std / 3.85e8) * np.ones((3,))) ** 2  # 50 meters
        P0_vel = ((vel_std/1.025e6)*np.ones((3,)))**2 #5 meters per second
        P0 = np.diag(np.hstack((P0_pos, P0_vel)))

        x0 = self.x0.reshape(6,1) + np.sqrt(P0)@np.random.normal(0, 1, (6,1))
        self.xhat = x0
        self.error = np.zeros((6,len(self.t_plot)))
        self.P = P0
        self.sigma = np.sqrt(np.diagonal(P0)).reshape(6,1)
        # print(self.P.shape)
        for k in range(0, len(self.t)):
            ...
            self.sigma = np.hstack((self.sigma,np.sqrt(np.diagonal(p1pre)).reshape(6,1), np.sqrt(np.diagonal(p1pos)).reshape(6,1)))
            # self.P = np.stack((p1pre,p1pos), axis = 1)
            # self.P[2*k:2*k+2,:,:] = np.stack((p1pre, p1pos), axis = 0)
        # print(self.sigma)
        self.xhat = self.xhat[:,1:]
        self.error[:,0::2] = self.Halo_orbit.T-self.xhat[:,::2]
        self.error[:,1::2] = self.Halo_orbit.T-self.xhat[:,1::2]
...
est = seq_est(X0, np.linspace(0,1*T,150))

#running estimator for monte carlo simulations
M = 100
mc_error = np.zeros((M, 6,len(est.t_plot)))
mc_sigma = np.zeros((M, 6,len(est.t_plot)))
est.t_plot = est.t_plot/sys.n/3600/24
for m in range(0,M):
    est.run()
    mc_error[m, :, :] = est.error
    mc_error[m,0:3,:] *= sys.r12*1000
    mc_error[m,3:,:] *= sys.n*sys.r12*1000*100
    mc_sigma[m, :, :] = est.sigma
    mc_sigma[m,0:3,:] *= sys.r12*1000
    mc_sigma[m,3:,:] *= sys.n*sys.r12*1000*100
...
#plotting
plt.figure(1, figsize = (6.5,5), tight_layout = True)
plt.subplot(321)
plt.suptitle('State component errors from true')
plt.subplot(321)
plt.plot(est.t_plot, error[:,0,:].T, color = 'c', linewidth = 0.85)
plt.plot(est.t_plot,  3*sigma_pos_avg[0,:], color = 'k', linestyle = '--')
plt.plot(est.t_plot, -3*sigma_pos_avg[0,:],color = 'k', linestyle = '--', label ='3$\sigma_x$')
plt.ylabel('X (m)', labelpad = 0)
plt.grid()
plt.legend()
plt.subplot(323)
plt.plot(est.t_plot,  error[:,1,:].T, color = 'c', linewidth = 0.85)
plt.plot(est.t_plot,  3*sigma_pos_avg[1,:], color = 'k', linestyle = '--')
plt.plot(est.t_plot, -3*sigma_pos_avg[1,:],color = 'k', linestyle = '--', label ='3$\sigma_y$')
plt.ylabel('Y (m)', labelpad = 0)
plt.grid()
plt.legend()
plt.subplot(325)
plt.plot(est.t_plot,  error[:,2,:].T, color = 'c', linewidth = 0.85)
plt.plot(est.t_plot,  3*sigma_pos_avg[2,:], color = 'k', linestyle = '--')
plt.plot(est.t_plot, -3*sigma_pos_avg[2,:],color = 'k', linestyle = '--', label ='3$\sigma_z$')
plt.xlabel('Time (days)', labelpad = 0)
plt.ylabel('Z (m)')
plt.grid()
plt.legend()

plt.subplot(322)
plt.plot(est.t_plot, error[:,3,:].T, color = 'c', linewidth = 0.85)
plt.plot(est.t_plot, 3*sigma_vel_avg[0,:], color = 'k', linestyle = '--')
plt.plot(est.t_plot, -3*sigma_vel_avg[0,:],color = 'k', linestyle = '--', label ='3$\sigma_\dot{x}$')
plt.ylabel('$\dot{X}$ (cm/s)', labelpad = 0)
plt.grid()
plt.legend()
plt.subplot(324)
plt.plot(est.t_plot, error[:,4,:].T, color = 'c', linewidth = 0.85)
plt.plot(est.t_plot, 3*sigma_vel_avg[1,:], color = 'k', linestyle = '--')
plt.plot(est.t_plot, -3*sigma_vel_avg[1,:],color = 'k', linestyle = '--', label ='3$\sigma_\dot{y}$')
plt.ylabel('$\dot{Y}$ (cm/s)', labelpad = 0)
plt.grid()
plt.legend()
plt.subplot(326)
plt.plot(est.t_plot, error[:,5,:].T, color = 'c', linewidth = 0.85)
plt.plot(est.t_plot, 3*sigma_vel_avg[2,:], color = 'k', linestyle = '--')
plt.plot(est.t_plot, -3*sigma_vel_avg[2,:],color = 'k', linestyle = '--', label ='3$\sigma_\dot{z}$')
plt.xlabel('Time (days)')
plt.ylabel('$\dot{Z}$ (cm/s)', labelpad = 0)
plt.grid()
plt.legend()

plt.figure(2, figsize = (6.5,5), tight_layout = True)
plt.suptitle('State component error std\'s vs average std\'s from true (Monte carlo)')
plt.subplot(321)
plt.plot(est.t_plot, error_std[0,:], color = 'b', label = 'Error std')
plt.plot(est.t_plot, sigma_pos_avg[0,:],color = 'k', linestyle = '--', label ='$\sigma_x$')
plt.ylabel('X (m)', labelpad = 0)
plt.grid()
plt.legend()
plt.subplot(323)
plt.plot(est.t_plot, error_std[1,:], color = 'b', label = 'Error std')
plt.plot(est.t_plot, sigma_pos_avg[1,:],color = 'k', linestyle = '--', label ='$\sigma_y$')
plt.ylabel('Y (m)', labelpad = 0)
plt.grid()
plt.legend()
plt.subplot(325)
plt.plot(est.t_plot, error_std[2,:], color = 'b', label = 'Error std')
plt.plot(est.t_plot, sigma_pos_avg[2,:],color = 'k', linestyle = '--', label ='$\sigma_z$')
plt.xlabel('Time (days)', labelpad = 0)
plt.ylabel('Z (m)')
plt.grid()
plt.legend()
...
plt.subplot(322)
plt.plot(est.t_plot, error_std[3,:], color = 'b', label = 'Error std')
plt.plot(est.t_plot, sigma_vel_avg[0,:],color = 'k', linestyle = '--', label ='$\sigma_\dot{x}$')
plt.ylabel('$\dot{X}$ (cm/s)', labelpad = 0)
plt.grid()
plt.legend()
plt.subplot(324)
plt.plot(est.t_plot, error_std[4,:], color = 'b', label = 'Error std')
plt.plot(est.t_plot, sigma_vel_avg[1,:],color = 'k', linestyle = '--', label ='$\sigma_\dot{y}$')
plt.ylabel('$\dot{Y}$ (cm/s)', labelpad = 0)
plt.grid()
plt.legend()
plt.subplot(326)
plt.plot(est.t_plot, error_std[5,:], color = 'b', label = 'Error std')
plt.plot(est.t_plot, sigma_vel_avg[2,:],color = 'k', linestyle = '--', label ='$\sigma_\dot{z}$')
plt.xlabel('Time (days)')
plt.ylabel('$\dot{Z}$ (cm/s)', labelpad = 0)
plt.grid()
plt.legend()

orbitplot , orbitax = plt.subplots(subplot_kw={"projection": "3d"})
x = est.Halo_orbit[:,:3]*3.85e5
orbitax.plot(x[:,0], x[:,1], x[:,2], linewidth = 1.5, label = '$L_1$ Halo Orbit ')
orbitax.scatter(sys.pi1*3.85e5, 0, 0, label = 'Moon')
orbitax.legend()
orbitax.set_xlabel('x (km)')
orbitax.set_ylabel('y (km)')
orbitax.set_zlabel('z (km)')
# orbitax.set_title('Halo Orbit $L_1$')
l1 = 0.8369180979439842
orbitax.set_xlim3d([(l1+sys.pi1)*3.85e5/2 - 55000, (l1+sys.pi1)*3.85e5/2 + 55000])
orbitax.set_ylim3d(-55000, 55000)
orbitax.set_zlim3d(-55000, 55000)
orbitax
plt.show()
