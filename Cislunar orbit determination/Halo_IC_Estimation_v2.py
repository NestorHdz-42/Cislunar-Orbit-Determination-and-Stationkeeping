...
def Jac(rho, rhodot, phi):
    ...
    for i in np.arange(0,len(d),1):
        ...
        jac[2 * i:2 * i + 2, :] = np.block([[drho_drho[i,:], drho_drhodot[i,:]],[drhodot_drho[i,:], drhodot_drhodot[i,:]]]) @ stm
    return jac
...
#setting up halo orbit parameters
...
for m in np.arange(0,M,1):
    ...
    x0c = X0.reshape(6, 1) + np.sqrt(P0g) @ np.random.normal(0, 1, (6, 1))
    ...
    x0c, Orbit_estimate, Jmin, i, H = halo_od(range_msd, range_rt_msd, W, 2*T, x0c.reshape((6,)))
...
#converting to dim units for plotting and calculating statistical values
error[:,0:3,:] *= sys.r12*1000
sigma_pos = np.sqrt(sigma_pos)*sys.r12*1000
error[:,3:,:] *= sys.n*sys.r12*1000*100
sigma_vel = np.sqrt(sigma_vel)*sys.n*sys.r12*1000*100
...
plt.plot(t, -3*sigma_pos_avg[:,0],color = 'k', linestyle = '--', label ='$\sigma_x$')
...
plt.plot(t, -3*sigma_pos_avg[:,1],color = 'k', linestyle = '--', label ='$\sigma_y$')
...
plt.plot(t, -3*sigma_pos_avg[:,2],color = 'k', linestyle = '--', label ='$\sigma_z$')
plt.xlabel('Time (days)')
plt.ylabel('Z (m)', labelpad = 0)
plt.grid()
plt.legend()
...
plt.plot(t, -3*sigma_vel_avg[:,0],color = 'k', linestyle = '--', label ='$\sigma_\dot{x}$')
...
plt.plot(t, -3*sigma_vel_avg[:,1],color = 'k', linestyle = '--', label ='$\sigma_\dot{y}$')
...
plt.plot(t, -3*sigma_vel_avg[:,2],color = 'k', linestyle = '--', label ='$\sigma_\dot{z}$')
plt.xlabel('Time (ND)')
